    ...
    def thread_scheduler(self, name):
        ...
        while self.__event.evt_close_app.is_set() == False:
            ...
            if (self.__event.evt_pause.is_set()) == True:
                ...
                while self.__event.evt_resume.is_set() is False:
                    time.sleep(self.CLOCK_PERIOD_SEC[0] / 20)  # polling with 1/10th the clock resolution shall be fine..
                if configuration.TEST:
                ...
                if self.clock_high == False:
                    while self.__event.evt_step_on.is_set() == True and self.__event.evt_do_step.is_set() == False:
                        ...
                        while self.__event.evt_clock.is_set() is False:
                            time.sleep(self.CLOCK_PERIOD_SEC[0] / 20)  # polling with 1/10th the clock resolution shall be fine..
                    if configuration.TEST:
                        self.test.prev_time = self.start_time
                else:
                ...
                if configuration.TEST:
                    ...
                    if tdiff != 0:
                        ...
                        ###############################################
                        # BUG: appending on string takes a lot of time!
                ...
                # BUG: event.wait has a delay of 10ms which limits the clock frequency to max. 100 Hz
                # self.__event.evt_clock.wait()
                while self.__event.evt_clock.is_set() is False:
                    time.sleep(self.CLOCK_PERIOD_SEC[0]/20)  # polling with 1/10th the clock resolution shall be fine..
                self.__event.evt_clock.clear()
